...
// Vendored from https://ui.elevenlabs.io/docs/components/orb (elevenlabs/ui registry).
// A WebGL sphere whose motion reacts to `agentState` (or manual input/output volume).
//
// react-three-fiber's Canvas is an intentional escape hatch from React's render model:
// useFrame mutates refs/shader uniforms in place on every animation frame (the standard,
// performant r3f pattern — re-rendering the component tree at 60fps would defeat the
// point) and useTexture returns a shared, mutable Three.js resource. The React Compiler's
// purity/immutability/ref-access lint rules assume plain React rendering and don't apply
// to this imperative loop, so they're disabled for this file rather than worked around.
/* eslint-disable react-hooks/purity, react-hooks/immutability, react-hooks/refs, react-hooks/preserve-manual-memoization */
