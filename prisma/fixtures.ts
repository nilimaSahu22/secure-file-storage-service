import { PrismaClient, Role, AppointmentStatus, PriorAuthStatus, ReferralStatus, AlertSeverity } from "@prisma/client";
...
const STAFF = [
  { key: "drRamirez", name: "Dr. Elena Ramirez", role: Role.DOCTOR, department: "Internal Medicine" },
  { key: "drChen", name: "Dr. Marcus Chen", role: Role.DOCTOR, department: "Cardiology" },
  { key: "nursePatel", name: "Nurse Aisha Patel", role: Role.NURSE, department: "Internal Medicine" },
  { key: "adminReyes", name: "Sam Reyes", role: Role.ADMIN, department: "Operations" },
] as const;
...
export async function seedDatabase(prisma: PrismaClient): Promise<void> {
  ...
  for (const s of STAFF) {
    const created = await prisma.staffUser.create({
      data: { name: s.name, role: s.role, department: s.department },
    });
